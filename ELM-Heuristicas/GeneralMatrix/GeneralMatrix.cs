...
namespace DotNetMatrix
{
	...
	[Serializable]
	public class GeneralMatrix : ICloneable, ISerializable, IDisposable, IEquatable<GeneralMatrix>
	{
        ...
        /// <param name="n">   Number of colums.
        /// </param>
        /// <param name="s">   Fill the matrix with random values less than cero.
        /// </param>

        public GeneralMatrix(int m, int n, Random r)
        {
            this.m = m;
            this.n = n;
            A = new double[m][];
        ...
        /// <summary>
        /// Sum of all values in a same column
        /// </summary>
        /// <param name="col">Column number to sumarize</param>
        /// <returns>Column sum</returns>
        public virtual double SumColumn(int col)
        {
            double sum = 0;
            for (var j = 0; j < n; j++)
            {
                sum += GetElement(j, col);
            }
            return sum;
        }

        /// <summary>
        /// Calcula la longitud del vector columna en la matriz
        /// </summary>
        /// <param name="col">Column number to calculate</param>
        /// <returns>Column sum</returns>
        public virtual double LengthColumnVector(int col)
        {
            double sum = 0;
            for (var i = 0; i < n; i++)
            {
                sum += Math.Pow(GetElement(i, col), 2);
            }
            sum = Math.Sqrt(sum);
            return sum;
        }
        ...
        /// <summary>
        /// Calcula la longitud del vector fila en la matriz
        /// </summary>
        /// <param name="row">Row number to calculate</param>
        /// <returns>Column sum</returns>
        public virtual double LengthRowVector(int row)
        {
            double sum = 0;
            for (int j = 0; j < n; j++)
            {
                sum += Math.Pow(GetElement(row, j), 2);
            }
            sum = Math.Sqrt(sum);
            return sum;
        }
        ...
        public virtual GeneralMatrix Sort(int posColumn)
        {
            var ordered = A.OrderByDescending(i => i[posColumn]);
            return new GeneralMatrix(ordered.ToArray());
        }
	    ...
	    {
	        if (ColumnDimension != other?.ColumnDimension) return false;
	        if (RowDimension != other.RowDimension) return false;
            for (var i = 0; i < RowDimension; i++)
                for (var j = 0; j < ColumnDimension; j++)
                    if (Math.Abs(A[i][j] - other.A[i][j]) > 1e-10) return false;
	        return true;
	    }
